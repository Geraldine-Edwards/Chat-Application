...
export const ALLOWED_ORIGINS = [
  "http://127.0.0.1:5500",
  "http://localhost:3000",
  "https://ge-chat-app-websockets-frontend.trainees.hosting.cyf.academy",
];
...
function validateAndSanitizeMessage(body, res) {
  ...
  if (!isValidMessageInput(message, sender)) {
    res.status(400).json({
      error:
        "Message and sender's name must be non-empty strings (max 300 chars for message, 50 for sender)",
    });
    return null;
  }
...
function broadcastNewMessage(newMsg) {
  ...
  waitingClients.forEach((client) =>
    client.res.json([formatMessageForClient(newMsg)]),
  );
